...
Both axes run as **parallel subagents** so they do not pollute each other's context, then this skill aggregates their findings. Use the Pi subagent skill (`~/.pi/agent/skills/subagent/SKILL.md`): start both `Agent` calls with `subagent_type: "Explore"` in the background, then retrieve both completed reports with `get_subagent_result`.
...
Start both `Agent` calls with `subagent_type: "Explore"` and `run_in_background: true`. Explicitly instruct each child to report findings without editing files. This is a task constraint, not an OS sandbox guarantee. Give each a self-contained prompt and use a short axis name as `description`.

Include the resolved requirements source or its fetched contents so both reviewers use the same version of the spec.
