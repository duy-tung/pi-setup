...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_tree", (_e, ctx) => {
		// The cache may contain an entry from the branch we just left. Refold the
		// selected branch immediately so get/create/update never sees abandoned state.
		cached = fold(ctx);
		roundInFlight = false;
		status(ctx);
	});
  ...
  pi.on("agent_settled", (_e, ctx) => {
    ...
    // A package completion/user follow-up already owns the next turn.
    if (ctx.hasPendingMessages?.() || ctx.isIdle?.() === false) return;
    driveRound(ctx, goal);
	});
