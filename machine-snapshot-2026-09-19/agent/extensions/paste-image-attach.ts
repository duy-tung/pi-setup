...
export function retireConsumedClipboardTags(
  pending: Map<number, string>,
  spans: { path: string; isTag: boolean; tagNumber?: number }[],
): void {
  for (const span of spans) {
    if (!span.isTag || span.tagNumber === undefined || !isClipboardTemp(span.path)) continue;
    if (pending.get(span.tagNumber) === span.path) pending.delete(span.tagNumber);
  }
}
...
export default function (pi: ExtensionAPI) {
   ...
   */
  // @ts-expect-error the private handlePaste below makes the subclass itself a
  // structural mismatch upstream; see the class doc for why that is deliberate.
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    // Pi resets the custom editor component (resetExtensionUI) before every
    // session invalidation (/new, /resume, /fork) and before /reload, so the
    // component is never still ours here and installing is always required.
    // Installing builds a new editor and copies only the text across; that
    // reset, not this extension, is what drops undo/history on those paths.
    ctx.ui.setEditorComponent(editorFactory);

    if (
      !warnedMissingHandlePaste &&
      typeof (CustomEditor.prototype as unknown as { handlePaste?: unknown }).handlePaste !== "function"
    ) {
  ...
  pi.on("input", async (event, ctx) => {
    ...
    type Span = { start: number; end: number; full: string; path: string; isTag: boolean; tagNumber?: number };
    const spans: Span[] = [];

    for (const m of event.text.matchAll(TAG_RE)) {
      const tagNumber = Number(m[1]);
      const path = pending.get(tagNumber);
      // A tag the user deleted, or one left over from a previous session, is
      // simply never resolved and stays as literal text.
      if (!path) continue;
      const start = m.index ?? 0;
      spans.push({ start, end: start + m[0].length, full: m[0], path, isTag: true, tagNumber });
    }
    ...
    // A submitted clipboard tag owns a one-shot temp file. Retire only those
    // exact mappings after all awaits. A new paste made while resizing remains
    // in the map, and non-clipboard files stay reusable across submissions.
    retireConsumedClipboardTags(pending, ordered);
