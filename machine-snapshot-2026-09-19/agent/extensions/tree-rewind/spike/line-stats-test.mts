...
const env = noIndexEnv("/ceiling", { PATH: "/bin", GIT_DIR: "/x", GIT_WORK_TREE: "/y", GIT_CONFIG_PARAMETERS: "'core.bigFileThreshold=1'", GIT_CONFIG_COUNT: "1", GIT_CONFIG_KEY_0: "core.bigFileThreshold", GIT_CONFIG_VALUE_0: "1", GIT_ATTR_SOURCE: "HEAD", GIT_EXTERNAL_DIFF: "/bin/false", GIT_CEILING_DIRECTORIES: "/elsewhere" });
assert.equal(env.PATH, "/bin");
for (const k of ["GIT_WORK_TREE", "GIT_CONFIG_PARAMETERS", "GIT_CONFIG_COUNT", "GIT_CONFIG_KEY_0", "GIT_CONFIG_VALUE_0", "GIT_ATTR_SOURCE", "GIT_EXTERNAL_DIFF"]) assert.equal(env[k], undefined, k);
assert.equal(env.GIT_CONFIG_GLOBAL, "/dev/null");
assert.equal(env.GIT_DIR, "/ceiling/no-repository", "no repository is ever discovered for the count");
...
try {
  ...
  // --- a delete and a create with identical bytes are two files, not a rename:
  // git's rename pairing would otherwise report both as ±0 under one name.
  const gone = item("gone-same.txt", "delete");
  const born = item("born-same.txt", "restore");
  const paired = await lineStats({ from: {}, to: {}, items: [gone, born] }, {
    current: (i) => (i === gone ? Buffer.from("same\n") : null),
    target: () => Buffer.from("same\n"),
  });
  assert.deepEqual(paired.get(key("gone-same.txt")), { kind: "lines", added: 0, removed: 1 });
  assert.deepEqual(paired.get(key("born-same.txt")), { kind: "lines", added: 1, removed: 0 });

  // --- a source that fails after the current side was staged leaves no half
  // pair behind for another item's file to be diffed against.
  const failing = item("failing.txt", "restore");
  const fine = item("fine.txt", "restore");
  const realWrite = fs.writeFileSync;
  let injected = false;
  fs.writeFileSync = ((path: any, ...rest: any[]) => {
    if (!injected && String(path).endsWith("/b/0")) { injected = true; throw Object.assign(new Error("ENOSPC"), { code: "ENOSPC" }); }
    return (realWrite as any)(path, ...rest);
  }) as typeof fs.writeFileSync;
  syncBuiltinESMExports();
  let halfStaged;
  try {
    halfStaged = await lineStats({ from: {}, to: {}, items: [failing, fine] }, {
      current: (i) => (i === failing ? Buffer.from("same\n") : null),
      target: (i) => Buffer.from(i === failing ? "other\n" : "same\n"),
    });
  } finally { fs.writeFileSync = realWrite; syncBuiltinESMExports(); }
  assert.ok(injected, "the temp write failure was injected");
  assert.equal(halfStaged.get(key("failing.txt"))?.kind, "unknown");
  assert.deepEqual(halfStaged.get(key("fine.txt")), { kind: "lines", added: 1, removed: 0 });
