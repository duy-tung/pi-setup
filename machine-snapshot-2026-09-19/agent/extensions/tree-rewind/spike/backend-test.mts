...
import { chmodSync, existsSync, lstatSync, mkdirSync, mkdtempSync, readFileSync, readdirSync, realpathSync, renameSync, rmSync, symlinkSync, writeFileSync, linkSync } from "node:fs";
...
async function testCaseCollision() {
  ...
  const renamed = newRepo("case-rename");
  writeFileSync(join(renamed, "case.txt"), "content\n");
  sh(renamed, "git", ["add", "-A"]);
  sh(renamed, "git", ["commit", "-qm", "i"]);
  const renameWorkspace = await open(renamed);
  if (renameWorkspace.coverage.caseInsensitive) {
    const lower = await renameWorkspace.snapshot(null, "lower");
    renameSync(join(renamed, "case.txt"), join(renamed, "CASE.txt"));
    const upper = await renameWorkspace.snapshot(lower, "upper");
    const renamePlan = await renameWorkspace.buildPlan(upper, lower);
    ok(renamePlan.items.length > 0, "case-only rename remains visible in the shadow diff");
    const renameResult = await renameWorkspace.apply(renamePlan);
    const names = readdirSync(renamed);
    ok(
      renameResult.errors.length === 0 && names.includes("case.txt") && !names.includes("CASE.txt"),
      `applying the plan restores the exact checkpoint casing (errors=${renameResult.errors.join(";") || "none"}, names=${names.join(",")})`,
    );
  }
}
...
async function testMaintenance() {
  ...
  ws.markSessionActive("old-session");
  const protectedRecent = await ws.maintain({ sessionId: "current-session", maxSessions: 1, maxAgeDays: 3650, recentSessionGraceMs: 0 });
  ok(!protectedRecent.prunedSessions.includes("old-session"), "count pressure preserves another process-active session lease");
  ws.releaseSession("old-session");
  const res = await ws.maintain({ sessionId: "current-session", maxSessions: 1, maxAgeDays: 3650, recentSessionGraceMs: 0 });
...
async function testMidSessionNestedRepo() {
  ...
  const unborn = join(d, "unborn");
  mkdirSync(unborn);
  sh(unborn, "git", ["init", "-q"]);
  writeFileSync(join(unborn, "x.txt"), "not committed\n");
  const cp3 = await ws.snapshot(cp2, "cp3", ["unborn/x.txt"]);
  ok(Boolean(cp3[""]), "an unborn nested repo does not fail the root snapshot");
  ok(ws.coverage.skippedNested.includes("unborn"), "an unborn nested repo is declared unprotected");
}
...
async function testAbsolutePathBreaksCheckpoint() {
  ...
  const snapshot = await ws.snapshot(null, "cp", [outside]);
  ok(Boolean(snapshot[""]), "a stale absolute force-track path is ignored instead of poisoning the checkpoint");
}
...
async function testProjectless() {
  ...
  const state = createInitialState();
  state.sessionId = "projectless-session";
...
async function testReaper() {
  ...
  // A missing project may be renamed or unmounted; its refs remain useful.
  rmSync(d, { recursive: true, force: true });
  reaped = reapStores(old);
  ok(reaped.length === 0, "a missing project's referenced store is retained");
  ok(existsSync(store), "checkpoints survive temporary origin absence");
  ...
  ok(reapStores(old).length === 0, "its checkpoint still prevents whole-store deletion");
  rmSync(join(live, "root.git", "refs"), { recursive: true, force: true });
  ok(reapStores(old).length === 1, "only the now ref-less store is swept");
...
async function testMissingNestedKeepsRootRestorable() {
  ...
  ok(await ws1.hasSnapshot(checkpoint), "checkpoint remains valid for repos still present");
  const now = await ws1.snapshot(null, "current");
  const again = await ws1.snapshot(now, "current again");
  ok(Boolean(again[""]), "the same Workspace remains usable on every later snapshot");
  const plan = await ws1.buildPlan(again, checkpoint);
  ok(plan.items.some((item) => item.action === "unprotected" && item.display === "vendor/dep"), "missing nested component is declared unprotected");
  const result = await ws1.apply(plan);
