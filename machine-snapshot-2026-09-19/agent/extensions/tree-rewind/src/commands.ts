...
import { consentFor, formatPlan, formatTotal, group, isEmpty, isOutsideItem, needsConfirmation, outsideItems, summarise } from "./plan.js";
...
function listUserPrompts(state: RewindState, ctx: any) {
  ...
  // The selector hands back a string, so two prompts with the same second and
  // the same first 68 characters must still be told apart by their label.
  const taken = new Set<string>();
  for (const p of out) {
    let label = p.label;
    for (let n = 2; taken.has(label); n++) label = `${p.label} (${n})`;
    taken.add(label);
    p.label = label;
  }
  return out.reverse();
 ...
 */
/** Why a file restore must not start now, or null. Inside Pi's own /tree or
 *  /fork navigation, `ctx.isIdle()` is false by construction: the host installs
 *  its branch-summary controller before it dispatches the hook, so "compacting"
 *  is true for the whole dialog. Pi has already refused to navigate while an
 *  agent run is active, so the idle check would only reject a legitimate
 *  restore there; the package-activity probe still applies. */
async function activityReason(state: RewindState, ctx: any): Promise<string | null> {
  const idle = () => state.hostNavigating || ctx.isIdle?.() !== false;
  const busy = "Wait for the active response to finish before restoring files.";
  if (!idle()) return busy;
  const reason = await state.restoreBlocker?.();
  // The probe is asynchronous; an agent run can begin while it is in flight.
  return reason ?? (idle() ? null : busy);
}

async function restoreBlocked(state: RewindState, ctx: any): Promise<boolean> {
  const alive = flowGuard(state);
  if (!alive()) return true;
  const reason = await activityReason(state, ctx);
  if (!alive()) return true;
...
async function restoreFilesChecked(state: RewindState, ctx: any, cp: PromptCheckpoint): Promise<boolean> {
  ...
  // Consent is bound to the items the dialogs listed. Apply re-plans under
  // the lock; anything that appeared since is skipped, never swept in.
  const result = await runBackend(state.lifetime, () => applyPlan(state, plan, { ...consentFor(plan, includeTypeChanges, includeOutside), activityCheck: () => activityReason(state, ctx) }));
  if (!alive()) return false;
...
async function doUndoChecked(state: RewindState, ctx: any): Promise<void> {
  ...
  const result = await runBackend(state.lifetime, () => applyUndo(state, prepared, { ...consentFor(prepared.plan, includeTypeChanges, true), activityCheck: () => activityReason(state, ctx) }));
  if (!alive()) return;
  reportApply(ctx, result, prepared.plan, stats);
}

/** Ignored paths are inventoried by name, never copied: bash writes there are
 *  not covered, and only write/edit-named files get a first-touch preimage. */
function ignoredCoverageLines(state: RewindState): string {
  const current = state.currentEntryId ? state.checkpoints.get(state.currentEntryId) : undefined;
  const latest = current ?? [...state.checkpoints.values()].sort((a, b) => b.timestamp - a.timestamp)[0];
  const unknown = (latest?.projectUnknown ?? []).filter((p) => p !== "");
  const rootUnknown = latest?.projectUnknown?.includes("") ?? false;
  const head = rootUnknown
    ? `  ignored inventory incomplete at the latest checkpoint: deletions under\n  ignored paths are ambiguous and will be skipped`
    : `  .gitignore'd paths: bash/manual writes there are not covered; only files\n  named by write/edit get a pre-write copy`;
  const list = unknown.length
    ? `\n  ${unknown.length} ignored path(s) present at the latest checkpoint:\n` +
      unknown.slice(0, 10).map((p) => `    ${p}`).join("\n") + (unknown.length > 10 ? `\n    … ${unknown.length - 10} more` : "")
    : "";
  return head + list;
}
...
async function offerRestore(
  ...
  state.hostNavigating = true;
  try { return await offerRestoreChecked(state, ctx, targetId, codeOnlyLabel, alive); }
  finally { if (alive()) state.hostNavigating = false; }
}

async function offerRestoreChecked(
  state: RewindState,
  ctx: any,
  targetId: string,
  codeOnlyLabel: string,
