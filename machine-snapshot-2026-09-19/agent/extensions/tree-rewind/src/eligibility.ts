 ...
 */
const SECRETISH = [
  // The documented policy is prefix/glob-shaped (`.env*`, `id_*`,
  // `credentials.*`): `.envrc`, `.env-prod`, `id_rsa-old` and
  // `credentials.csv` are refused, not just the canonical spellings.
  /^\.env/i,
  /^\.netrc$/i,
  /^\.npmrc$/i,
  /^\.pypirc$/i,
