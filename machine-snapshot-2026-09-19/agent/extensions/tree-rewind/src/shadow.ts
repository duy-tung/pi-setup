...
/** Git's object id of a blob: `blob <size>\0<bytes>` under the store's hash. */
function hashObject(body: Buffer, idLength: number): string {
  return createHash(idLength === 64 ? "sha256" : "sha1").update(`blob ${body.length}\0`).update(body).digest("hex");
}
...
export class ShadowRepo {
  ...
  private run(args: string[], input?: string | Buffer, maxOutputBytes?: number): Promise<GitResult> {
    return runGit(args, this.env, { input, signal: this.signal, maxOutputBytes });
  }
  ...
  async commit(parents: string[], message: string, forceTrack?: Iterable<string>): Promise<string> {
    ...
    // Git 2.55 says "<sha> is not a valid object"; older versions said
    // "not a valid object name". Match the shared stem.
    if (r.code !== 0 && parents.length > 0 && /not a valid object|bad object|invalid object/i.test(r.stderr)) {
  ...
  async sessionRefs(): Promise<Map<string, number>> {
    ...
    // An enumeration failure must not read as "no sessions": maintenance would
    // treat every other shadow's refs as dead.
    if (r.code !== 0) throw new ShadowError("for-each-ref", r.stderr);
  ...
  /** Which of `shas` are not intact blobs in this store. One batch call, and
   *  `--batch-check` reads the object header, so a truncated loose object is
   *  reported as missing rather than trusted. */
  async missingBlobs(shas: Iterable<string>): Promise<string[]> {
    const wanted = [...new Set(shas)].filter((sha) => OBJECT_ID_RE.test(sha));
    if (!wanted.length) return [];
    const r = await this.run(["cat-file", "--batch-check=%(objectname) %(objecttype) %(objectsize)"], wanted.join("\n") + "\n");
    if (r.code !== 0) return wanted;
    const sizes = new Map<string, number>();
    for (const line of text(r).split("\n")) {
      const m = /^([0-9a-f]{40,64}) blob (\d+)$/.exec(line.trim());
      if (m) sizes.set(m[1], Number(m[2]));
    }
    const missing = wanted.filter((sha) => !sizes.has(sha));
    // The header check proves the object exists; only re-hashing the payload
    // proves it is intact. Read in bounded batches; oversize blobs one by one.
    const present = wanted.filter((sha) => sizes.has(sha));
    const BATCH = 48 * 1024 * 1024;
    let batch: string[] = []; let batchBytes = 0;
    const flush = async () => {
      if (!batch.length) return;
      const shas = batch; batch = []; batchBytes = 0;
      const out = await this.run(["cat-file", "--batch"], shas.join("\n") + "\n");
      if (out.code !== 0) { missing.push(...shas); return; }
      let offset = 0; const buf = out.stdout;
      for (const sha of shas) {
        const nl = buf.indexOf(0x0a, offset);
        const header = nl < 0 ? "" : buf.subarray(offset, nl).toString("utf8");
        const m = /^([0-9a-f]{40,64}) blob (\d+)$/.exec(header);
        if (!m || m[1] !== sha) { missing.push(...shas.slice(shas.indexOf(sha))); return; }
        const size = Number(m[2]); const body = buf.subarray(nl + 1, nl + 1 + size);
        offset = nl + 1 + size + 1;
        if (body.length !== size || hashObject(body, sha.length) !== sha) missing.push(sha);
      }
    };
    // Framing per record: "<sha> blob <size>\n" + body + "\n"; count it so the
    // batch output never exceeds the subprocess capture bound.
    const framed = (sha: string, size: number) => size + sha.length + ` blob ${size}\n`.length + 1;
    for (const sha of present) {
      const size = framed(sha, sizes.get(sha)!);
      if (size > BATCH) {
        await flush();
        const raw = sizes.get(sha)!;
        const one = await this.run(["cat-file", "blob", sha], undefined, raw + 1024 * 1024);
        if (one.code !== 0 || one.stdout.length !== raw || hashObject(one.stdout, sha.length) !== sha) missing.push(sha);
        continue;
      }
      if (batchBytes + size > BATCH) await flush();
      batch.push(sha); batchBytes += size;
    }
    await flush();
    return missing;
  }
