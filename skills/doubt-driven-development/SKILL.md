...
This is not post-hoc code review (see /Users/tung/.pi/agent/skills/code-review/SKILL.md) — that is a verdict on a finished artifact. This is an in-flight posture: non-trivial decisions get cross-examined while course-correction is still cheap.
...
This skill is designed for the **main session**, where Step 3 (DOUBT, detailed below) can spawn a fresh-context reviewer. pi sub-agents cannot spawn sub-agents; if you are already a sub-agent, surface to the user that doubt-driven cannot run nested rather than faking a self-review.
...
Spawn the reviewer with the pi subagent skill (/Users/tung/.pi/agent/skills/subagent/SKILL.md): `agent_spawn` with role `reviewer` (read-only — it physically cannot "fix" things mid-review), mode `oneshot`, and the adversarial prompt above pasted verbatim as the `task`. The sub-agent starts with fresh context by design and knows nothing of this conversation — exactly what doubt-driven needs. Collect with `agent_wait`.
...
| "I'll do doubt at the end with a code review" | Code review is a final gate. Doubt-driven catches wrong directions early when course-correction is cheap. By PR time it's too late. |
...
- Doubting only after committing — that's post-hoc code review, not doubt-driven development
...
- **code-review** (/Users/tung/.pi/agent/skills/code-review/SKILL.md): complementary. That is a post-hoc verdict on a finished diff; doubt-driven is in-flight per-decision. Use both.
- **tdd** (/Users/tung/.pi/agent/skills/tdd/SKILL.md): TDD's RED step is doubt made concrete — a failing test is a disproof attempt. When TDD applies, that failing test *is* the doubt step for behavioral claims.
- **diagnosing-bugs** (/Users/tung/.pi/agent/skills/diagnosing-bugs/SKILL.md): when the reviewer surfaces a real failure mode, drop into that skill to localize and fix.
