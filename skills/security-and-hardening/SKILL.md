...
These are prevention patterns, not a ranking. For the 2021 ordering, see the quick-reference table in `security-checklist.md` (in this skill's folder).
...
When data crosses a trust boundary, validate it as untrusted (see Input Validation above); when a privacy incident exposes personal data, the breach-notification clock is part of the postmortem — follow /Users/tung/.pi/agent/skills/diagnosing-bugs/SKILL.md.
...
For detailed security checklists and pre-commit verification steps, see `security-checklist.md` in this skill's folder.
