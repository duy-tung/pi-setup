...
export async function beginRestoreTransaction(
  ...
  options: { includeOutside?: boolean; includeTypeChanges?: boolean }, ws: Workspace | null,
  ...
  for (const item of plan.items) {
    if (item.repo !== OUTSIDE || !options.includeOutside || item.action === "unprotected" ||
        (item.action === "type-change" && !options.includeTypeChanges)) continue;
