...
export async function lineStats(
  ...
  try {
    dir = mkdtempSync(join(tmpdir(), "pi-rewind-stat-"));
    const a = join(dir, "a");
    const b = join(dir, "b");
    mkdirSync(a);
    mkdirSync(b);
    ...
    const r = await runGit(["diff", "--no-index", "--numstat", "-z", "--", a, b], noIndexEnv(), { cwd: dir, signal });
...
export function noIndexEnv(base: NodeJS.ProcessEnv = process.env): NodeJS.ProcessEnv {
